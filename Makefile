...
export CFLAGS  := -Oz -flto -I ./deps/rapidjson/include -I./deps/littlefs -fno-exceptions -include ./config.h
...
dist/index.mjs: memfs.wasm
	sed -i 's/^class Asyncify/export class Asyncify/g' ./deps/asyncify/asyncify.mjs
#	$(shell npm bin)/tsc -p ./tsconfig.json
#	$(shell npm bin)/esbuild --bundle ./index.ts --outfile=$@ --format=esm --log-level=warning --external:*.wasm
