...
class CallFrame {
  public:
    template<class T> std::span<T> alloc_uninitialized(const std::size_t count);
    template<class T, class U>
    std::span<T> ref_array(const U addr, const std::size_t count);
...
#define IMPORT(x) __attribute__((__import_module__("internal"), __import_name__(#x))) x
...
template<class T> class MutableView {
  public:
    explicit MutableView(CallFrame& frame, const int32_t addr, const int32_t count = 1)
      : value(frame.ref_array<T>(addr, count))
      , addr(addr) {
    }
    T& get() {
      return value[0];
...
template<class T, class U>
[[nodiscard]] std::span<T> CallFrame::ref_array(const U addr, const std::size_t count) {
...
template<class T>
[[nodiscard]] std::span<T> CallFrame::alloc_uninitialized(const std::size_t count) {
