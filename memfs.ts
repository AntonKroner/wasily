...
export class MemFS {
	...
	constructor(preopens: Array<string>, fs: _FS) {
		this.#instance = new WebAssembly.Instance(wasm, {
			internal: {
				...
				trace: (isError: number, addr: number, size: number) => {
					const view = new Uint8Array(this.#getInternalView().buffer, addr, size)
				...
				copy_out: (srcAddr: number, dstAddr: number, size: number) => {
					...
					const src = new Uint8Array(this.#getInternalView().buffer, srcAddr, size)
					dst.set(src)
				},
				copy_in: (srcAddr: number, dstAddr: number, size: number) => {
					const src = new Uint8Array(this.#hostMemory!.buffer, srcAddr, size)
					const dst = new Uint8Array(this.#getInternalView().buffer, dstAddr, size)
					dst.set(src)
				},
			},
		...
		// eslint-disable-next-line @typescript-eslint/ban-types
		const initialize_internal = this.#instance.exports.initialize_internal as Function
	...
	#copyFrom(src: Uint8Array): number {
		// eslint-disable-next-line @typescript-eslint/ban-types
		const dstAddr = (this.#instance.exports.allocate as Function)(src.byteLength)
		new Uint8Array(this.#getInternalView().buffer, dstAddr, src.byteLength).set(src)
		return dstAddr
	}
}
